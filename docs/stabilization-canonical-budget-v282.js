...
let me=null,profile={role:'user'},timer=null,busy=false;
...
function normalize(){document.querySelectorAll('.pill,.mgmt-chip,option,.next-step').forEach(el=>setText(el,el.textContent.replace(/Finance Processing|Financial Processing/g,'Reimbursement Processing')));['totalHero','sideTotal'].forEach(id=>{const el=$(id);if(el&&/^[A-Z]{3}\s/.test(el.textContent))setText(el,el.textContent.replace(/^[A-Z]{3}\s/,'HKD '))})}
async function verifyBudget(){if(busy||!me||!['admin','manager','finance'].includes(profile.role))return;const fy=$('budgetFySelect')?.value;if(!fy)return;busy=true;try{const [rs,bs]=await Promise.all([getDocs(collection(db,'expenseReports')),getDocs(collection(db,'budgets'))]);const reports=rs.docs.map(d=>d.data()),budgets=bs.docs.map(d=>d.data());document.querySelectorAll('#budgetRows input[id^="budget-"]').forEach(inp=>{const uid=inp.id.slice(7),b=budgets.find(x=>x.userId===uid&&x.fiscalYear===fy),amount=+(b?.budgetAmount||0),mine=reports.filter(r=>r.userId===uid&&fyOf(r)===fy),used=mine.filter(r=>USED.has(r.status)).reduce((s,r)=>s+total(r),0),pending=mine.filter(r=>r.status==='Submitted').reduce((s,r)=>s+total(r),0),cells=inp.closest('tr')?.querySelectorAll('td');if(cells?.length>=6){setText(cells[3],money(used));setText(cells[4],money(pending));setText(cells[5],money(amount-used-pending))}})}catch(e){console.warn('Budget verification failed',e.message)}finally{busy=false}}
...
new MutationObserver(m=>{if(!m.some(x=>x.addedNodes.length||x.removedNodes.length))return;clearTimeout(timer);timer=setTimeout(run,400)}).observe(document.body,{childList:true,subtree:true});