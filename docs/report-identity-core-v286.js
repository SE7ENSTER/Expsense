...
function formItems(){
  return [...document.querySelectorAll('#items .item')].map((item,index)=>{
    ...
    return {
      ...
      date:inputs[0]?.value||'',
      desc:inputs[1]?.value||'',
      category:selects[0]?.value||'Transportation',
      currency:selects[1]?.value||'HKD',
      amount:inputs[2]?.value||'',
      fx:Number(inputs[3]?.value||1),
      rateSource:selects[2]?.value||prior.rateSource||'XE manual',
      note:text?.value||'',
      receiptData:image,
      receiptName:prior.receiptName||''
    };
  });
}
...
function payload(status){
  ...
  const totalHkd=items.reduce((sum,x)=>sum+(Number(x.amount)||0)*(Number(x.fx)||0),0);
  return {
    ...(activeSnapshot||{}),
    id:activeDocId||activeSnapshot?.id||crypto.randomUUID(),
    status,
    userId:activeSnapshot?.userId||user.uid,
    userEmail:activeSnapshot?.userEmail||user.email,
    meta:{
    ...
    createdAt:activeSnapshot?.createdAt||serverTimestamp(),
    submittedAt:status==='Submitted'?(activeSnapshot?.submittedAt||serverTimestamp()):(activeSnapshot?.submittedAt||null)
  };
}
...
async function loadSnapshot(docId){
  ...
  const snap=await getDoc(doc(db,'expenseReports',docId));
...
async function saveWithStableIdentity(status){
  ...
  try{
    ...
    if(!targetId){
      const ref=doc(collection(db,'expenseReports'));
      targetId=ref.id;
    ...
    console.error('Stable save failed',e);
...
function patch(){
  ...
  window.openReport=async id=>{
    ...
    activeDocId=docId;
    activeSnapshot=await loadSnapshot(docId);
    if(activeSnapshot&&activeSnapshot.id!==docId){
      await setDoc(doc(db,'expenseReports',docId),{id:docId,docId},{merge:true});
      activeSnapshot.id=docId;
    }
    return oldOpen(id);
  };
...
setTimeout(patch,2100);
const patchTimer=setInterval(()=>{patch();if(patched)clearInterval(patchTimer)},500);
