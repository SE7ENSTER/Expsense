...
function buildTable(){
  ...
  if(!cards.length){
    wrap.innerHTML='<div class="report-grid-empty">No reports found.</div>';
    return;
  }
  const rows=cards.map(parseCard);
  wrap.innerHTML=`<div class="report-table-scroll"><table class="report-grid"><thead><tr><th>Status</th><th>Purpose / Trip</th><th>User</th><th>Total</th><th>Items</th><th>Next Step</th><th>Action</th></tr></thead><tbody>${rows.map((r,i)=>`<tr data-report-row="${i}"><td><span class="pill ${r.status==='Submitted'?'submitted':''}">${escapeHtml(r.status)}</span></td><td class="cell-purpose">${escapeHtml(r.purpose)}<span class="cell-sub">${escapeHtml(r.trip)}</span></td><td>${escapeHtml(r.email)}</td><td class="cell-total">${escapeHtml(r.amount)}</td><td>${escapeHtml(r.items)}</td><td class="cell-next">${escapeHtml(r.next)}</td><td><button class="secondary" type="button" data-report-open="${i}">Open</button></td></tr>`).join('')}</tbody></table></div>`;
  wrap.querySelectorAll('[data-report-row]').forEach(tr=>{
    tr.addEventListener('click',ev=>{
      if(ev.target.closest('button'))return;
      const r=rows[Number(tr.dataset.reportRow)];
      if(r?.id&&window.openReport)window.openReport(r.id);
      else if(r?.click)Function(r.click)();
    });
  });
  wrap.querySelectorAll('[data-report-open]').forEach(btn=>{
    btn.addEventListener('click',ev=>{
      ev.stopPropagation();
      const r=rows[Number(btn.dataset.reportOpen)];
      if(r?.id&&window.openReport)window.openReport(r.id);
      else if(r?.click)Function(r.click)();
    });
  });
...
function injectToggle(){
  ...
  if(!toolbar)return;
  const titleBox=toolbar.querySelector('div');
  if(titleBox&&!titleBox.classList.contains('report-title-wrap'))titleBox.classList.add('report-title-wrap');
  if(!document.getElementById('reportViewToggle')){
    const toggle=document.createElement('div');
    toggle.id='reportViewToggle';
    toggle.className='report-view-toggle';
    toggle.innerHTML='<button type="button" data-report-view="cards">Cards</button><button type="button" data-report-view="table">Table</button>';
    if(titleBox)titleBox.after(toggle); else toolbar.prepend(toggle);
    toggle.querySelectorAll('button').forEach(btn=>btn.addEventListener('click',()=>setMode(btn.dataset.reportView)));
  }
  applyMode(preferredMode());
}
function patchViewChanges(){
  const oldShow=window.showView;
  if(oldShow&&!window.__reportViewShowPatched){
    window.__reportViewShowPatched=true;
    window.showView=view=>{
      oldShow(view);
      if(view==='dashboard')setTimeout(injectToggle,450);
    };
  }
  const oldRefresh=window.refreshAdmin;
  if(oldRefresh&&!window.__reportViewRefreshPatched){
    window.__reportViewRefreshPatched=true;
...
function observeReports(){
  const list=document.getElementById('reportList');
  if(!list||list.__reportViewObserved)return;
  list.__reportViewObserved=true;
  new MutationObserver(()=>{
    clearTimeout(reportViewObserverTimer);
    reportViewObserverTimer=setTimeout(()=>applyMode(preferredMode()),250);
  }).observe(list,{childList:true,subtree:true});
}
function init(){patchViewChanges();injectToggle();observeReports();}
setTimeout(init,1700);
document.addEventListener('click',()=>setTimeout(()=>{injectToggle();observeReports()},250),true);
console.log('ExpenseFlow report view toggle v27 active');
